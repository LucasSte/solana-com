...
Solana. Precisamos do `solana-program` para o SDK principal e do `borsh` para
serialização.
...
Todo programa Solana tem um ponto de entrada, que é a função que é chamada
...
Este padrão de roteamento é comum em programas Solana. O `instruction_data` é
...
Parabéns! Você construiu um programa Solana completo que demonstra a estrutura
básica compartilhada por todos os programas Solana:
...
O LiteSVM simula o ambiente de execução da Solana, permitindo testar nosso
